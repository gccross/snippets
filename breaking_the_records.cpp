...
vector<int> breakingRecords(vector<int> scores) {
    int highs{0}, lows{0}, high{*scores.begin()}, low{*scores.begin()};
    for_each(scores.begin(), scores.end(), 
        [&lows,&highs, &high, &low](int i){
            if (i > high) { high = i; ++highs; return; }
            if (i < low) { low = i; ++lows; return; }
        });
    
    return vector<int>{ highs, lows };
}
